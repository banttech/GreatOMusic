  ...
  <div class="container-xxl flex-grow-1 container-p-y">
      ...
      <!-- <form action="{{ route('slider.search') }}" method="GET">
        <div class="input-group mb-3">
          <input type="text" class="form-control" placeholder="Search by slider name" name="search" value="{{ request()->search }}">
          ...
          @if(count($sliders) > 0)
            @foreach($sliders as $key => $slider)
              <tr>
                ...
                <td>{{ $slider->text}}</td>
                <td>{{ $slider->image}}</td>
                <td> <a href="{{ route('slider.edit', $slider->id) }}" class="btn btn-primary btn-sm">Edit</a>
                <a href="{{ route('slider.delete',$slider->id) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this slider content?');">Delete</a>
                </td>
              </tr> 
            @endforeach
          @else
