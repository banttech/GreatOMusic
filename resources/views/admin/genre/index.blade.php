  ...
  <div class="container-xxl flex-grow-1 container-p-y">
      ...
      <!-- <form action="{{ route('genre.search') }}" method="GET">
        <div class="input-group mb-3">
          <input type="text" class="form-control" placeholder="Search by genre name" name="search" value="{{ request()->search }}">
        ...
        @if(count($genres) > 0)
          @foreach($genres as $key => $genre)
            <tr>
              ...
              <td>{{ $genre->name }}</td>
              <td> <a href="{{ route('genre.edit', $genre->id) }}" class="btn btn-primary btn-sm">Edit</a>
              <a href="{{ route('genre.delete',$genre->id) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this genre?');">Delete</a>
              </td>
            </tr>
          @endforeach
        @else
